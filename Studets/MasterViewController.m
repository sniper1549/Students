...
- (void) synchronizeWithServer
{
    NSURL *storeURL = [[(AppDelegate*)[[UIApplication sharedApplication] delegate] applicationDocumentsDirectory] URLByAppendingPathComponent:@"Students.sqlite"];
   ...
   /* NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:kServerUrl]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        if (data)
        {
            self.students = data == nil ? nil : [data objectFromJSONData];
            ...
            for(int i=0;i<[self.students count];i++){
                Student *stud = [NSEntityDescription insertNewObjectForEntityForName:@"Student" inManagedObjectContext:context];
                
                [stud setValuesForKeysWithDictionary:[self.students objectAtIndex:i]];
            }
            
            NSError *error;
            if(![context save:&error])
            {
                NSLog(@"moo foo bar error");
            }
        }
        else
        {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.detailViewController.curentStudent = [[self fetchedResultsController] objectAtIndexPath:indexPath];
    [self.navigationController pushViewController:self.detailViewController animated:YES];
}
